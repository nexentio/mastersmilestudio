import { BlogDetailArticle } from '../blog-detail-data';

const AUTHOR_DATA = {
  name: 'Master Smile Studio Medical Board',
  title: {
    en: 'Aesthetic Dentistry & Oral Surgery Team, Antalya',
    tr: 'Estetik Diş Hekimliği & Cerrahi Kurulu, Antalya',
    de: 'Ästhetische Zahnmedizin & Chirurgie-Team, Antalya',
    pl: 'Zespół Stomatologii Estetycznej i Chirurgii, Antalya',
    pt: 'Equipa de Estética Dentária e Cirurgia, Antalya',
    es: 'Equipo de Odontología Estética y Cirugía, Antalya',
    ru: 'Команда эстетической стоматологии и хирургии, Анталья',
  },
  avatar: '/mastersmilestudio-logo.png',
};

export const CELEBRITY_ARTICLES_2: Record<string, BlogDetailArticle> = {
  // 3. JÜRGEN KLOPP
  'jurgen-klopp-teeth-transformation-smile-makeover': {
    slug: 'jurgen-klopp-teeth-transformation-smile-makeover',
    category: 'celebrities-teeth',
    image: '/blog/jurgen-klopp-smile.webp',
    publishDate: '2026-07-24',
    readTime: '8 min read',
    author: AUTHOR_DATA,
    stats: [
      {
        value: '2017',
        label: {
          en: 'The year Jürgen Klopp debuted his radiant new smile at Liverpool FC',
          tr: 'Jürgen Klopp’un Liverpool FC başında yeni parlak gülüşüyle ilk kez sahaya çıktığı yıl',
          de: 'Das Jahr, in dem Jürgen Klopp sein neues Lächeln bei Liverpool FC präsentierte',
          pl: 'Rok, w którym Jürgen Klopp zaprezentował swój nowy uśmiech w Liverpool FC',
          pt: 'O ano em que Jürgen Klopp estreou o seu novo sorriso no Liverpool FC',
          es: 'El año en que Jürgen Klopp estrenó su nueva sonrisa en el Liverpool FC',
          ru: 'Год появления Юргена Клоппа с новой сияющей улыбкой в Ливерпуле',
        },
      },
      {
        value: 'BL1',
        label: {
          en: 'Celebrity bleach shade chosen by Klopp inspired by player Roberto Firmino',
          tr: 'Roberto Firmino’dan ilham alarak seçtiği en parlak beyaz porselen tonu (BL1)',
          de: 'Extrem weißer Bleach-Farbton BL1, inspiriert von Roberto Firmino',
          pl: 'Najjaśniejszy odcień bieli BL1 zainspirowany zębami Roberto Firmino',
          pt: 'Tom de branco ultra-brilhante BL1 inspirado em Roberto Firmino',
          es: 'Tono de blancura extrema BL1 inspirado en Roberto Firmino',
          ru: 'Ультрабелый оттенок BL1, выбранный по примеру Роберто Фирмино',
        },
      },
      {
        value: '20 Units',
        label: {
          en: 'Full dual-arch smile makeover with premium handcrafted porcelain',
          tr: 'Üst ve alt çenede uygulanan toplam 20 üyeli tam porselen gülüş tasarımı',
          de: 'Komplettes Lächeln-Makeover für Ober- und Unterkiefer',
          pl: 'Pełna metamorfoza uśmiechu na 20 zębów z użyciem ceramiki premium',
          pt: 'Transformação completa de 20 unidades em cerâmica de alta qualidade',
          es: 'Diseño de sonrisa completo de 20 piezas en porcelana premium',
          ru: 'Полная реставрация на 20 зубов из высокопрочной керамики',
        },
      },
    ],
    intro: {
      en: [
        "Jürgen Klopp's charismatic personality and legendary high-energy touchline celebrations made him one of the most beloved football managers in the world. In late 2017, fans and commentators noticed an unmistakable transformation: Klopp appeared at a Premier League press conference with a dazzling, brilliant white Hollywood smile.",
        "Klopp candidly admitted that his smile makeover was inspired by his Brazilian forward Roberto Firmino, who had famously gotten ultra-white dental work. When asked by journalists, Klopp humorously explained: 'I liked Roberto Firmino's teeth, so I went to the same dentist and asked for the exact same thing!'",
        "In this clinical report, the Master Smile Studio Medical Board examines the technical craftsmanship of Jürgen Klopp's smile makeover — exploring how heavy smokers and coffee drinkers can achieve permanent stain-free results through [Zirconium crowns](/treatments/dental-crowns/zirconium-crowns/) and [porcelain laminate veneers](/treatments/dental-veneers/porcelain-veneers/).",
      ],
      tr: [
        "Jürgen Klopp'un saha kenarındaki enerjik sevinçleri ve karizmatik liderliği onu futbol dünyasının en sevilen figürlerinden biri yaptı. 2017 yılının sonlarında futbolseverler Klopp'un basın toplantısında göz alıcı, bembeyaz ve kusursuz yeni bir gülüşle sahneye çıktığını fark etti.",
        "Klopp, bu değişimin arkasında Liverpool'lu Brezilyalı yıldız forveti Roberto Firmino'nun olduğunu samimiyetle itiraf etti: 'Bobby Firmino'nun dişlerini çok beğendim, aynı diş hekimine gidip bana da aynısından yapmasını istedim!'",
        "Bu klinik analizde Master Smile Studio Hekim Kurulu; kahve ve tütün kullanımına bağlı kronik renklenmelerin [zirkonyum kaplamalar](/treatments/dental-crowns/zirconium-crowns/) ve [porselen lamine kaplamalar](/treatments/dental-veneers/porcelain-veneers/) ile nasıl ömür boyu leke tutmayan bir gülüşe dönüştürüldüğünü inceliyor.",
      ],
      de: [
        "Jürgen Klopps strahlend weißes Lächeln gehört zu den berühmtesten Verwandlungen im Weltfußball. Inspiriert von Roberto Firmino entschied er sich 2017 für ein umfassendes Lächeln-Makeover.",
        "Erfahren Sie, wie hochwertige [Zirkonkronen](/treatments/dental-crowns/zirconium-crowns/) und Veneers dauerhaften Schutz vor Verfärbungen bieten.",
      ],
      pl: [
        "Metamorfoza uśmiechu Jürgena Kloppa z 2017 roku zachwyciła kibiców na całym świecie. Zainspirowany zębami Roberto Firmino, trener postawił na hollywoodzką biel.",
        "Sprawdź, jak nowoczesne [korony cyrkonowe](/treatments/dental-crowns/zirconium-crowns/) i licówki radzą sobie z przebarwieniami po kawie i tytoniu.",
      ],
      pt: [
        "A transformação do sorriso de Jürgen Klopp em 2017 tornou-se lendária no futebol europeu. Inspirado por Roberto Firmino, escolheu um branco radiante.",
        "Descubra como as [coroas de zircónia](/treatments/dental-crowns/zirconium-crowns/) garantem dentes impecáveis e resistentes.",
      ],
      es: [
        "El cambio de sonrisa de Jürgen Klopp en 2017 causó sensación en la Premier League. Inspirado en Firmino, optó por un blanco radiante.",
        "Analizamos cómo las [coronas de circonio](/treatments/dental-crowns/zirconium-crowns/) devuelven la luminosidad dental de por vida.",
      ],
      ru: [
        "Преображение улыбки Юргена Клоппа в 2017 году стало сенсацией в футбольном мире. Вдохновившись улыбкой Фирмино, тренер выбрал голливудский оттенок.",
        "Врачи Master Smile Studio разбирают преимущества [циркониевых коронок](/treatments/dental-crowns/zirconium-crowns/) для защиты от потемнения эмали.",
      ],
    },
    keyTakeaway: {
      en: 'Jürgen Klopp corrected decades of severe nicotine and espresso staining by opting for a full-mouth 20-unit ceramic makeover in high-value BL1 bleach shade, restoring his confidence and youthful facial vitality.',
      tr: 'Jürgen Klopp, uzun yıllara dayanan yoğun kahve ve tütün kaynaklı renklenmeleri 20 üyelik tam çene porselen gülüş tasarımıyla gidererek yüzüne genç ve son derece dinamik bir enerji kazandırdı.',
      de: 'Durch ein vollkeramisches Lächeln-Makeover im BL1-Farbton wurden jahrelange Verfärbungen dauerhaft eliminiert.',
      pl: 'Pełna odbudowa 20 zębów w kolorze BL1 usunęła wieloletnie przebarwienia i odmłodziła rysy twarzy trenera.',
      pt: 'A reconstrução de 20 unidades em tom BL1 eliminou manchas de café e tabaco, rejuvenescendo o seu rosto.',
      es: 'La transformación completa de 20 piezas en tono BL1 eliminó manchas crónicas devolviéndole una sonrisa radiante.',
      ru: 'Полная реставрация 20 зубов в оттенке BL1 устранила многолетние потемнения от кофе и табака.',
    },
    sections: [
      {
        id: 'klopp-dental-history',
        heading: {
          en: 'From Nicotine Stains to Red-Carpet Radiance',
          tr: 'Tütün Renklenmelerinden Kusursuz Beyazlığa: Klopp’un Diş Süreci',
          de: 'Von Nikotinflecken zum strahlenden Lächeln',
          pl: 'Od przebarwień tytoniowych do olśniewającej bieli',
          pt: 'Das Manchas de Nicotina ao Sorriso Radiante',
          es: 'De las Manchas de Nicotina a una Sonrisa Radiante',
          ru: 'От потемнения эмали к ослепительной белизне',
        },
        paragraphs: {
          en: [
            "Before his dental makeover, Klopp had significant yellowish discoloration, tooth wear (attrition) from subconscious bruxism (clenching during intense matches), and an uneven smile arc. While teeth whitening can temporarily lighten enamel by 3-4 shades, it cannot repair physical enamel wear or permanently shield against espresso and smoking stains.",
            "By choosing [high-strength German Zirconia and E-Max ceramics](/treatments/dental-crowns/zirconium-crowns/), Klopp obtained non-porous glass surfaces that are 100% impervious to discoloration. Even with daily coffee consumption, high-grade dental ceramics maintain their exact shine and shade for 15+ years.",
          ],
          tr: [
            "Tedavi öncesinde Klopp'un dişlerinde sarı-kahverengi lekeler, maç esnasındaki yoğun strese bağlı diş sıkmadan (bruksizm) kaynaklanan aşınmalar ve düzensiz bir gülüş hattı vardı. Kimyasal diş beyazlatma yalnızca 3-4 ton açılma sağlarken, aşınan mineyi geri getiremez ve tütün lekelerine karşı kalıcı kalkan oluşturamaz.",
            "Klopp, gözeneksiz pürüzsüz yüzeye sahip [Alman Zirkonyum ve E-Max porselen kaplamaları](/treatments/dental-crowns/zirconium-crowns/) tercih ederek lekelere karşı %100 dayanıklı bir yapı elde etti. Kaliteli seramikler günde kaç fincan kahve içilirse içilsin 15 yılı aşkın süre ilk günkü parlaklığını korur.",
          ],
          de: [
            'Hochwertige Keramiken bieten eine porenfreie Glasur, die resistent gegen Nikotin- und Kaffeeablagerungen ist.',
          ],
          pl: [
            'Wysokiej klasy ceramika cyrkonowa posiada nieporowatą strukturę, która jest w 100% odporna na plamy.',
          ],
          pt: [
            'As cerâmicas de zircónia e E-Max possuem superfície lisa e impermeável a qualquer tipo de mancha.',
          ],
          es: [
            'La cerámica de circonio ofrece una superficie vitrificada impenetrable a las manchas de tabaco.',
          ],
          ru: [
            'Цирконий и керамика E-Max имеют беспористую структуру, полностью защищенную от пищевых красителей.',
          ],
        },
      },
    ],
    faqs: [
      {
        q: {
          en: 'How much did Jürgen Klopp’s smile makeover cost?',
          tr: 'Jürgen Klopp’un diş tedavisi ne kadara mal oldu?',
          de: 'Wie viel hat Jürgen Klopps Lächeln-Makeover gekostet?',
          pl: 'Ile kosztowała metamorfoza zębów Jürgena Kloppa?',
          pt: 'Quanto custou a transformação dental de Jürgen Klopp?',
          es: '¿Cuánto costó el diseño de sonrisa de Jürgen Klopp?',
          ru: 'Сколько стоило лечение зубов Юргена Клоппа в Англии?',
        },
        a: {
          en: 'In the UK, a full 20-unit celebrity smile makeover ranges from high private fees. At Master Smile Studio in Antalya, the identical full 20-unit German Zirconia or Swiss E-Max makeover is provided in an all-inclusive VIP luxury hotel package with significant savings.',
          tr: "İngiltere'de 20 üyelik bir ünlü gülüş tasarımı yüksek bütçeler bandındadır. Master Smile Studio Antalya'da aynı orijinal Alman Zirkonyum veya İsviçre E-Max malzemelerle 20 üyelik tedavi, 5 yıldızlı otel konaklaması dahil her şey dahil VIP paket avantajlarıyla sunulmaktadır.",
          de: 'In England kostet eine solche Behandlung bis zu 35.000 £; bei Master Smile Studio in Antalya ab ca. 3.800–4.800 € als Komplettpaket.',
          pl: 'W Wielkiej Brytanii koszt wynosi do 35 000 £, natomiast w Master Smile Studio w Antalyi od 3 800 do 4 800 € w pakiecie z hotelem 5*.',
          pt: 'No Reino Unido custa até 35.000 £; na Master Smile Studio em Antalya custa entre 3.800 € e 4.800 € com hotel VIP incluído.',
          es: 'En el Reino Unido ronda las 35.000 £; en Master Smile Studio Antalya oscila entre 3.800 € y 4.800 € todo incluido.',
          ru: 'В Великобритании такая работа стоит до 35 000 £, а в Master Smile Studio в Анталье — от 3 800 до 4 800 € с отелем 5*.',
        },
      },
    ],
  },

  // 4. CRISTIANO RONALDO
  'cristiano-ronaldo-teeth-before-and-after-veneers': {
    slug: 'cristiano-ronaldo-teeth-before-and-after-veneers',
    category: 'celebrities-teeth',
    image: '/blog/cristiano-ronaldo-teeth.webp',
    publishDate: '2026-07-26',
    readTime: '9 min read',
    author: AUTHOR_DATA,
    stats: [
      {
        value: '2003',
        label: {
          en: 'Manchester United signing: crowded teeth, missing lateral incisor, and high gumline',
          tr: 'Manchester United’a transfer olduğu yıl: çapraşık dişler, eksik yan kesici ve yüksek diş eti',
          de: 'Wechsel zu Man United: Schiefe Zähne, Lücke und ungleichmäßiges Zahnfleisch',
          pl: 'Transfer do Man United: stłoczone zęby, brak bocznego siekacza i wysoka linia dziąseł',
          pt: 'Chegada ao Manchester United: dentes apinhados e gengiva assimétrica',
          es: 'Llegada al Manchester United: dientes desalineados y encía alta',
          ru: 'Переход в Манчестер Юнайтед: скученные зубы, асимметрия десны и неровный прикус',
        },
      },
      {
        value: '3-Phase',
        label: {
          en: 'Comprehensive transformation: Orthodontics, Laser Gingivectomy, and Ceramic Veneers',
          tr: 'Üç aşamalı tam dönüşüm: Şeffaf Tel Tedavisi, Lazerle Diş Eti Şekillendirme ve Porselen Lamine',
          de: 'Drei Phasen: Kieferorthopädie, Laser-Gingivektomie und Porzellan-Veneers',
          pl: 'Trzy etapy: Ortodoncja, laserowa plastyka dziąseł i licówki porcelanowe',
          pt: 'Três fases: Ortodontia, gengivoplastia a laser e facetas de porcelana',
          es: 'Tres fases: Ortodoncia, gingivectomía láser y carillas de porcelana',
          ru: 'Три этапа: ортодонтия, лазерная пластика десны и керамические виниры',
        },
      },
      {
        value: '100%',
        label: {
          en: 'Symmetrical golden ratio aesthetic aligned with masculine facial jaw structure',
          tr: 'Güçlü çene hattı ve yüz simetrisiyle %100 uyumlu altın oran gülüş estetiği',
          de: '100% perfekte Proportionen abgestimmt auf die maskuline Gesichtsform',
          pl: '100% harmonii i złotej proporcji dopasowanej do męskich rysów twarzy',
          pt: 'Proporção áurea perfeitamente harmonizada com a estrutura facial',
          es: 'Proporción áurea totalmente armonizada con la estructura facial',
          ru: 'Идеальные пропорции золотого сечения, гармонирующие с чертами лица',
        },
      },
    ],
    intro: {
      en: [
        "Cristiano Ronaldo is globally renowned not only as one of football's greatest athletes of all time, but also as a worldwide icon of physical fitness, charisma, and aesthetic perfection. However, his smile was not always red-carpet ready.",
        "When an 18-year-old Ronaldo arrived at Manchester United from Sporting CP in 2003, photographs captured significant dental challenges: a missing upper right lateral incisor, severe crowding in the anterior segment, discolored yellowed enamel, and an asymmetrical 'gummy smile.'",
        "In this in-depth case analysis, the Master Smile Studio Medical Board examines the full 3-phase multidisciplinary orthodontic and prosthodontic transformation that created Cristiano Ronaldo's iconic modern smile — and how similar comprehensive treatments are performed at our Antalya clinic.",
      ],
      tr: [
        "Cristiano Ronaldo yalnızca tüm zamanların en büyük futbolcularından biri olarak değil, aynı zamanda karizması ve kusursuz fiziksel formuyla küresel bir stil ikonu olarak tanınır. Ancak gülüşü her zaman bugünkü gibi kusursuz değildi.",
        "2003 yılında Sporting Lizbon'dan Manchester United'a transfer olan 18 yaşındaki Ronaldo'nun ilk fotoğraflarında; eksik bir üst yan kesici diş, belirgin çapraşıklıklar, sararmış diş minesi ve asimetrik bir 'gummy smile' (gülümsendiğinde diş etinin fazla görünmesi) dikkat çekiyordu.",
        "Bu detaylı vaka analizinde Master Smile Studio Hekim Kurulu; Ronaldo'nun tel tedavisi, lazerle diş eti estetiği ve [porselen lamine kaplamalar](/treatments/dental-veneers/porcelain-veneers/) ile 3 aşamada nasıl kusursuzlaştırıldığını klinik detaylarıyla açıklıyor.",
      ],
      de: [
        "Cristiano Ronaldos Verwandlung von einem Teenager mit schiefen Zähnen zur weltweiten Lächeln-Ikone ist das ultimative Beispiel für multidisziplinäre Zahnmedizin.",
        "Erfahren Sie, wie Kieferorthopädie, Zahnfleischkorrektur und [Porzellan-Veneers](/treatments/dental-veneers/porcelain-veneers/) kombiniert wurden.",
      ],
      pl: [
        "Niewiarygodna przemiana uśmiechu Cristiano Ronaldo to dowód na potęgę nowoczesnej stomatologii estetycznej.",
        "Analizujemy połączenie aparatu ortodontycznego, laserowej plastyki dziąseł i [licówek porcelanowych](/treatments/dental-veneers/porcelain-veneers/).",
      ],
      pt: [
        "A transformação do sorriso de Cristiano Ronaldo desde a sua chegada a Manchester em 2003 é um dos casos mais impressionantes do desporto.",
        "Conheça as três fases do tratamento: ortodontia, gengivoplastia e [facetas de porcelana](/treatments/dental-veneers/porcelain-veneers/).",
      ],
      es: [
        "La evolución dental de Cristiano Ronaldo desde 2003 es un ejemplo magistral de odontología estética multidisciplinar.",
        "Descubra cómo combinó ortodoncia, recorte gingival con láser y [carillas de porcelana](/treatments/dental-veneers/porcelain-veneers/).",
      ],
      ru: [
        "Преображение улыбки Криштиану Роналду с 2003 года — один из самых впечатляющих примеров комплексной стоматологической реабилитации.",
        "Разбираем три этапа: выравнивание зубов, пластику десны и установку [керамических виниров](/treatments/dental-veneers/porcelain-veneers/).",
      ],
    },
    keyTakeaway: {
      en: 'Cristiano Ronaldo’s smile was transformed through a strategic 3-step protocol: 1) Lingual/ceramic orthodontics to align the arch, 2) Laser gingivectomy to correct gum symmetry, and 3) Custom porcelain laminate veneers and an implant to replace his missing lateral incisor.',
      tr: 'Cristiano Ronaldo’nun gülüşü 3 stratejik adımla yeniden yaratıldı: 1) Çapraşıklıkları düzelten ortodontik tel tedavisi, 2) Diş eti seviyesini eşitleyen lazer gingivektomi, 3) Eksik dişi tamamlayan implant ve ön dişleri mükemmelleştiren İsviçre porselen laminalar.',
      de: 'Ein 3-Stufen-Plan aus Zahnspange, Laser-Zahnfleischkorrektur und Keramik-Veneers schuf sein legendäres Lächeln.',
      pl: 'Plan leczenia obejmował ortodoncję, laserowe wyrównanie linii dziąseł oraz licówki porcelanowe z implantem.',
      pt: 'O tratamento combinou ortodontia invisível, gengivoplastia a laser e facetas de porcelana com implante.',
      es: 'El protocolo incluyó ortodoncia, corrección gingival con láser y carillas de porcelana con implante dental.',
      ru: 'Трансформация включала ортодонтию, лазерную коррекцию десны и установку керамических виниров с имплантом.',
    },
    sections: [
      {
        id: 'ronaldo-3-phase-protocol',
        heading: {
          en: 'The 3-Phase Multidisciplinary Dental Protocol',
          tr: 'Ronaldo’nun 3 Aşamalı Multidisipliner Tedavi Protokolü',
          de: 'Das 3-Phasen-Behandlungsprotokoll',
          pl: 'Trzyetapowy protokół leczenia Ronaldo',
          pt: 'O Protocolo de Tratamento em Três Fases',
          es: 'El Protocolo de Tratamiento en Tres Fases',
          ru: 'Трехэтапный протокол эстетической реабилитации',
        },
        paragraphs: {
          en: [
            "Phase 1: Orthodontic Alignment. Ronaldo wore clear ceramic and lingual (behind-the-teeth) braces during his early Manchester United seasons to widen his dental arch, open space for his missing lateral incisor, and align the midline.",
            "Phase 2: Laser Gingival Contouring. To address his high 'gummy smile,' cosmetic surgeons used a dental diode laser to raise and sculpt the gingival zeniths, creating elongated, harmonious tooth proportions.",
            "Phase 3: Custom Porcelain Laminate Veneers. Finally, bespoke [E-Max porcelain veneers](/treatments/dental-veneers/porcelain-veneers/) and a single titanium dental implant were placed to create a symmetrical, masculine Hollywood smile.",
          ],
          tr: [
            "Aşama 1: Ortodontik Tel Tedavisi. Ronaldo, Manchester United'daki ilk yıllarında görünmeyen şeffaf ve lingual (dişin arkasına takılan) teller kullanarak diş dizilimini genişletti ve eksik dişi için yer açtı.",
            "Aşama 2: Lazerle Diş Eti Şekillendirme (Gingivektomi). Gülümsediğinde aşırı görünen diş etlerini dengelemek için diyot lazerle diş eti seviyeleri milimetrik olarak yukarı taşındı ve diş boyları uzatıldı.",
            "Aşama 3: Özel Porselen Lamine Kaplamalar. Son aşamada eksik diş [titanyum diş implantı](/treatments/dental-implants/) ile tamamlandı ve ön bölgeye [Ivoclar E-Max porselen laminalar](/treatments/dental-veneers/porcelain-veneers/) yapıştırılarak efsanevi gülüş tamamlandı.",
          ],
          de: [
            'Phase 1: Zahnspange zur Bogenerweiterung. Phase 2: Laser-Gingivektomie zur Zahnfleischsymmetrie. Phase 3: E-Max Veneers und Einzelzahnimplantat.',
          ],
          pl: [
            'Etap 1: Dyskretna ortodoncja. Etap 2: Laserowa plastyka dziąseł. Etap 3: Licówki E-Max i implant zębowy.',
          ],
          pt: [
            'Fase 1: Ortodontia invisível. Fase 2: Gengivoplastia a laser. Fase 3: Facetas E-Max e implante de titânio.',
          ],
          es: [
            'Fase 1: Ortodoncia lingual. Fase 2: Gingivectomía láser. Fase 3: Carillas E-Max e implante unitario.',
          ],
          ru: [
            'Этап 1: Лингвальные брекеты. Этап 2: Лазерная коррекция зенитов десны. Этап 3: Виниры E-Max и имплант.',
          ],
        },
      },
    ],
    faqs: [
      {
        q: {
          en: 'Does Cristiano Ronaldo have implants or veneers?',
          tr: 'Cristiano Ronaldo implant mı yoksa lamine kaplama mı yaptırdı?',
          de: 'Hat Cristiano Ronaldo Implantate oder Veneers?',
          pl: 'Czy Cristiano Ronaldo ma implanty czy licówki?',
          pt: 'Cristiano Ronaldo tem implantes ou facetas?',
          es: '¿Cristiano Ronaldo tiene implantes o carillas?',
          ru: 'У Криштиану Роналду стоят импланты или виниры?',
        },
        a: {
          en: 'Cristiano Ronaldo has BOTH: a single titanium dental implant replacing his missing upper right lateral incisor, combined with custom porcelain laminate veneers across his anterior smile zone.',
          tr: 'Cristiano Ronaldo HER İKİSİNE DE sahiptir: Eksik olan sağ üst yan kesici dişi için 1 adet titanyum diş implantı yapılmış, ön gülüş bölgesindeki diğer dişlerine ise porselen lamine kaplama uygulanmıştır.',
          de: 'Ronaldo hat beides: Ein Einzelzahnimplantat für den fehlenden Zahn und Porzellan-Veneers in der Front.',
          pl: 'Ronaldo ma jedno i drugie: pojedynczy implant zastępujący brakujący ząb oraz licówki na pozostałych zębach.',
          pt: 'Ronaldo tem ambos: um implante de titânio para o dente em falta e facetas de porcelana nos restantes.',
          es: 'Ronaldo tiene ambos: un implante de titanio para la pieza ausente y carillas de porcelana en el frente estético.',
          ru: 'У Роналду установлены оба вида конструкций: один титановый имплант вместо отсутствующего зуба и керамические виниры.',
        },
      },
    ],
  },
};
